...
- If you are running v0.8.7.4 as distributed by magnetocoin.org you do not need to upgrade.
  The binaries distributed on magnetocoin.org contain their own copy of openssl so they are unaffected by this issue.
...
- Minor fixes:
  - s/Bitcoin/Magnetocoin/ in the Coin Control example
...
- Internal miner added back to Magnetocoin.  setgenerate now works, although it is generally a bad idea as it is significantly slower than external CPU miners.
...
Fix out-of-bounds check (Magnetocoin currently does not use this codepath, but we apply this
patch just to match Bitcoin 0.8.5.)

0.8.4.1 changes
===============

CVE-2013-5700 Bloom: filter crash issue - Magnetocoin 0.8.3.7 disabled bloom by default so was 
...
nodes running with the -debug flag to crash, although it was lot less likely on Magnetocoin 
